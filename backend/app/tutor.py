...
def offline_answer(entries: list[tuple[KBEntry, float]]) -> str:
    ...
    if not entries or entries[0][1] < RELEVANCE_FLOOR:
        return ("I don't have anything on that yet. Try asking about one of the missions — "
                "grounding, hallucination, tokens and cost, the context window, embeddings, "
                "prompt injection, comparing models, system prompts, streaming, or source bias.")
    entry, _ = entries[0]
