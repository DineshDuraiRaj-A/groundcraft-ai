...
async def free_models():
    return {"models": await get_free_openrouter_models()}
...
async def upload_document(
    ...
    response.set_cookie("ts_session", session_id, httponly=True, samesite="lax")
...
async def query(req: QueryRequest, response: Response, ts_session: Optional[str] = Cookie(None)):
    ...
    if req.provider == "free":
        allowed, _remaining = ratelimit.check_and_increment(session_id)
        if not allowed:
            raise HTTPException(
                429,
                "You've hit the free-tier session limit for this hour. "
...
async def stats():
    ...
    return {"visitors_today": count, "storage": "supabase" if storage.SUPABASE_ENABLED else "in-memory"}


@app.post("/api/visit")
async def record_visit():
    count = await storage.record_visit()
...
def embedding_map(req: EmbeddingMapRequest):
    ...
    try:
        return embeddings_viz.compute_embedding_map(req.terms)
...
def context_preview(req: ContextPreviewRequest, ts_session: Optional[str] = Cookie(None)):
    ...
    if not chunks:
        raise HTTPException(400, "Document produced no chunks — is it empty?")
    index = TfidfIndex(chunks)
    ranked = index.query(req.question, top_k=len(chunks))  # rank ALL chunks, let the wall decide what fits
    return context_window.simulate_context_stuffing(ranked, req.question, req.max_context_tokens)
...
async def injection_test(req: InjectionTestRequest, response: Response, ts_session: Optional[str] = Cookie(None)):
    ...
    if req.provider == "free":
        allowed, _ = ratelimit.check_and_increment(session_id)
        if not allowed:
            raise HTTPException(429, "Free-tier session limit reached. Add your own key to keep testing.")
...
async def compare_models(req: CompareRequest, response: Response, ts_session: Optional[str] = Cookie(None)):
    ...
    if req.provider == "free":
        allowed, _ = ratelimit.check_and_increment(session_id)
        if not allowed:
            raise HTTPException(429, "Free-tier session limit reached. Add your own key to keep comparing.")
...
async def bias_probe(req: BiasProbeRequest, response: Response, ts_session: Optional[str] = Cookie(None)):
    ...
    if req.provider == "free":
        allowed, _ = ratelimit.check_and_increment(session_id)
        if not allowed:
            raise HTTPException(429, "Free-tier session limit reached. Add your own key to keep probing.")
...
async def query_stream(req: StreamQueryRequest, response: Response, ts_session: Optional[str] = Cookie(None)):
    ...
    if req.provider == "free":
        allowed, _ = ratelimit.check_and_increment(session_id)
        if not allowed:
            raise HTTPException(429, "Free-tier session limit reached. Add your own key to keep streaming.")
...
async def auth_google(req: GoogleAuthRequest, response: Response):
    ...
    response.set_cookie("gc_session", token, httponly=True, samesite="lax", max_age=60 * 60 * 24 * 30)
...
async def save_progress(req: ProgressRequest, gc_session: Optional[str] = Cookie(None)):
    user = auth.get_user(gc_session)
    if not user:
        # Guest mode: nothing to persist, but don't error — the frontend
        # keeps progress in memory and simply won't survive a reload.
        return {"persisted": False, "reason": "guest"}
...
async def assistant(req: AssistantRequest, response: Response, ts_session: Optional[str] = Cookie(None)):
    ...
    if req.provider == "free":
        allowed, remaining = ratelimit.check_and_increment(session_id, bucket="assistant")
        if not allowed:
...
async def full_status():
    ...
    is an LLM actually reachable, and how many people are here right now.
    """
    llm_ready = bool(os.environ.get("OPENROUTER_API_KEY") or os.environ.get("CEREBRAS_API_KEY"))
    return {
        "server": "online",
        "llm_configured": llm_ready,
        "storage": "supabase" if storage.SUPABASE_ENABLED else "in-memory",
