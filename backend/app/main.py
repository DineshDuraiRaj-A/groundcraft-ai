...
from . import auth, context_window, doc_preview, documents, embeddings_viz, ratelimit, storage, tutor
...
_PLACEHOLDER_WORDS = ("Curious", "Keen", "Sharp", "Bright", "Quiet", "Steady",
                      "Bold", "Patient", "Careful", "Quick")


def _placeholder_name(session_id: str) -> str:
    """A stable, friendly stand-in so nameless guests still make the board."""
    h = sum(ord(ch) for ch in session_id)
    return f"{_PLACEHOLDER_WORDS[h % len(_PLACEHOLDER_WORDS)]} Learner {h % 900 + 100}"
...
async def save_progress(req: ProgressRequest, response: Response,
    ...
    # Guests hold a leaderboard place too, keyed on their browser session.
    # No email is collected or stored for guests.
    #
    # This used to require a guest_name and silently DISCARD progress without
    # one — so anyone who skipped the optional name prompt could finish all ten
    # missions and never appear on the leaderboard. Progress is now always
    # saved; a friendly placeholder name is assigned if none was given, and
    # setting a name later simply renames the same row.
    sid = _get_or_create_session(ts_session)
    _set_session_cookie(response, "ts_session", sid)
    key = "guest:" + sid

    name = (req.guest_name or "").strip()[:28]
    if not name:
        existing = await storage.load_progress(key)
        name = (existing or {}).get("name") or _placeholder_name(sid)

    await storage.save_progress(key, name, "", req.xp, req.completed)
    return {"persisted": True, "xp": req.xp, "completed": req.completed,
            "as": "guest", "name": name}
...
def doc_preview_endpoint(req: DocPreviewRequest, response: Response,
                         ts_session: Optional[str] = Cookie(None)):
    """
    What's in this document, and what could I usefully ask about it?

    No LLM call: it runs instantly inside the document picker and costs
    nothing, which also means it still works when the AI quota is spent.
    """
    session_id = _get_or_create_session(ts_session)
    _set_session_cookie(response, "ts_session", session_id)
    ...
    if req.doc_source == "sample":
        if not req.sample_doc_id:
            raise HTTPException(400, "sample_doc_id is required")
        try:
            text = documents.load_sample_doc(req.sample_doc_id)
            title = documents.SAMPLE_DOCS[req.sample_doc_id]["title"]
        except KeyError:
            raise HTTPException(404, f"Unknown sample_doc_id '{req.sample_doc_id}'")
    else:
        text = _uploaded_docs.get(session_id)
        if not text:
            raise HTTPException(400, "No uploaded document found for this session.")
        title = "Your uploaded document"

    return doc_preview.preview(text, title)


@app.get("/api/debug/storage")
async def debug_storage():
    """
    Open this in a browser to see exactly what Supabase says about each table.
    Added because a blocked INSERT used to look identical to "no visitors yet".
    Reports permissions only — never row contents.
    """
    if not storage.SUPABASE_ENABLED:
        return {"supabase": "not configured",
                "hint": "Set SUPABASE_URL and SUPABASE_KEY in Render, then redeploy."}

    import httpx as _httpx
    out: dict = {"supabase": "configured", "url_host": storage.SUPABASE_URL.split("//")[-1][:40],
                 "degraded": not storage.supabase_live(),
                 "last_error": storage.storage_note(), "tables": {}}

    headers = {"apikey": storage.SUPABASE_KEY,
               "Authorization": f"Bearer {storage.SUPABASE_KEY}",
               "Content-Type": "application/json"}

    async with _httpx.AsyncClient(timeout=10.0) as client:
        for table in ("visits", "feedback", "progress"):
            entry: dict = {}
            try:
                r = await client.get(f"{storage.SUPABASE_URL}/rest/v1/{table}",
                                     headers={**headers, "Prefer": "count=exact", "Range": "0-0"},
                                     params={"select": "id" if table != "progress" else "user_sub"})
                entry["read"] = "ok" if r.status_code < 400 else f"BLOCKED {r.status_code}"
                if r.status_code >= 400:
                    entry["read_error"] = r.text[:180]
                else:
                    rng = r.headers.get("content-range", "")
                    entry["rows"] = int(rng.split("/")[-1]) if "/" in rng and rng.split("/")[-1].isdigit() else None
            except Exception as e:
                entry["read"] = f"ERROR: {str(e)[:120]}"
            out["tables"][table] = entry

        # a real insert into visits, since that is the one that was failing
        try:
            w = await client.post(f"{storage.SUPABASE_URL}/rest/v1/visits", headers=headers, json={})
            out["tables"]["visits"]["write"] = "ok" if w.status_code < 400 else f"BLOCKED {w.status_code}"
            if w.status_code >= 400:
                out["tables"]["visits"]["write_error"] = w.text[:180]
                out["fix"] = ("Row Level Security is blocking anonymous INSERT on `visits`. "
                              "In Supabase: Authentication -> Policies -> visits -> New Policy -> "
                              "allow INSERT and SELECT for the `anon` role. Do the same for `feedback`. "
                              "Leave `progress` backend-only: it holds email addresses.")
        except Exception as e:
            out["tables"]["visits"]["write"] = f"ERROR: {str(e)[:120]}"
