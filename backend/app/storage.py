...
async def record_visit(session_id: str | None = None) -> int:
    """
    Count each visitor once PER DAY, not once per request and not once ever.

    Deduping forever meant a returning visitor never registered again, so the
    number looked frozen. The key is session+date, so coming back tomorrow
    counts as a new visit while refreshing ten times today does not.
    """
    global _memory_visit_count

    if session_id:
        import datetime
        day_key = f"{session_id}:{datetime.date.today().isoformat()}"
        if day_key in _counted_sessions:
            return await get_visit_count()
        _counted_sessions.add(day_key)
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            # The insert result used to be discarded. If RLS blocked the anon
            # INSERT, the write failed silently with a 403, the read then
            # returned zero rows, and the app cheerfully reported "0 visitors"
            # with no error and no fallback. Check it.
            ins = await client.post(f"{SUPABASE_URL}/rest/v1/visits",
                                    headers=_headers(), json={})
            if ins.status_code >= 400:
                raise RuntimeError(
                    f"visits INSERT rejected ({ins.status_code}): {ins.text[:160]}"
                )
            return await _count_rows(client, "visits")
    except Exception as e:
        _note_supabase_failure(e)
        _memory_visit_count += 1
        return _memory_visit_count
...
async def _count_rows(client: "httpx.AsyncClient", table: str) -> int:
    """
    Ask Postgres for the count instead of downloading every row to len() it.
    Supabase returns it in Content-Range as "0-24/1234".
    """
    headers = _headers()
    ...
    if resp.status_code >= 400:
        raise RuntimeError(f"{table} SELECT rejected ({resp.status_code}): {resp.text[:160]}")
