...
async def stream_generate(
    ...
    if provider == "free":
        # pick the first configured provider that has a usable model, so
        # streaming works even when OpenRouter's free tier is exhausted
        chosen = None
        for prov in FREE_PROVIDERS:
            k = os.environ.get(prov["env"])
            if not k:
                continue
            mods = await _provider_models(prov)
            if mods:
                chosen = (prov, k, model or mods[0])
                break
        if not chosen:
            raise ProviderError("No AI provider is configured for streaming.")
        prov, key, chosen_model = chosen
        url = prov["base"]
